class CutScreen {
    private Browser = Laya.Browser;
    private Event = Laya.Event;
    private HTMLCanvas = Laya.HTMLCanvas;
    private Texture = Laya.Texture;
    private sp: Laya.Sprite;
    private CutCallBack: Laya.Handler;
    constructor() {
        // this.LayaAirDemo();
    }
    /**
    ...
    public CutScreen(sprite, width, height, offsetX, offsetY, callBack?: Laya.Handler) {
        ...
        let _texture: Laya.Texture = new this.Texture(htmlC);
    ...
    private onClick(): void {
        ...
        //此处将canvas指定区域进行截屏
        var htmlC: Laya.HTMLCanvas = this.sp.drawToCanvas(50, 45, 0, 0);
        //获取截屏区域的texture
        var _texture: Laya.Texture = new this.Texture(htmlC);
