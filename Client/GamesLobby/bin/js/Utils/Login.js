...
function GetWeChatShareDto(parentID, isAuthorize = true) {
    ...
    if (isAuthorize) {
        dto.Link = `https://open.weixin.qq.com/connect/oauth2/authorize?appid=${AppId}&redirect_uri=http%3A%2F%2F${Domain}%2Fgameshareplatform%2F%3fparentid%3d${parentID}&response_type=code&scope=snsapi_userinfo&state=123#wechat_redirect`;
    } else {
        dto.Link = `http://${Domain}/gameshareplatform/?parentid=${parentID}`;
    }
    return dto;
}
