 ...
 */
var NumberGradualChangeEffect = /** @class */ (function () {
    ...
    NumberGradualChangeEffect.prototype.initCounter = function (data) {
        this.changedCount = 1;
        this.result += data;
        this.cardinalNumber = Math.floor((this.result - this.showText) / this.count);
        if (this.cardinalNumber === 0) {
            this.allCount = 0;
        }
        else {
            this.allCount = Math.floor((this.result - this.showText) / this.cardinalNumber);
        }
    ...
    NumberGradualChangeEffect.prototype.onLoop = function () {
        ...
        var formatNum = Utils.Money.Format(this.showText, 0);
    ...
    NumberGradualChangeEffect.prototype.end = function () {
        var formatNum = Utils.Money.Format(this.result, 0);
        this.txt.changeText(formatNum);
        Laya.timer.clear(this, this.onLoop);
    };
