var __extends = (this && this.__extends) || (function () {
    var extendStatics = Object.setPrototypeOf ||
        ({ __proto__: [] } instanceof Array && function (d, b) { d.__proto__ = b; }) ||
        function (d, b) { for (var p in b) if (b.hasOwnProperty(p)) d[p] = b[p]; };
    return function (d, b) {
        extendStatics(d, b);
        function __() { this.constructor = d; }
        d.prototype = b === null ? Object.create(b) : (__.prototype = b.prototype, new __());
    };
})();
/// <reference path='ISocketManager.ts'/>
/// <reference path='EventEnum.ts'/>
/**
 * Socket管理类
 */
var SocketManager;
(function (SocketManager) {
    /**
     * Socket类
     */
    var WebSocket = /** @class */ (function (_super) {
        __extends(WebSocket, _super);
        function WebSocket() {
            var _this = _super.call(this) || this;
            _this.socket = new Utils.Socket.WebSocket();
            var params = new Utils.Socket.WebSocketParam();
            params.OnConnect = Laya.Handler.create(_this, _this.OnConnect, null, false);
            params.OnError = Laya.Handler.create(_this, _this.OnError, null, false);
            params.OnClosed = Laya.Handler.create(_this, _this.OnClosed, null, false);
            params.OnMessage = Laya.Handler.create(_this, _this.OnMessage, null, false);
            params.OnWillReconnect = Laya.Handler.create(_this, _this.OnWillReconnect, null, false);
            _this.socket.Init(params);
            return _this;
        }
        /**
         * 连接
         * @param token
         */
        WebSocket.prototype.Connect = function (url) {
            //启动连接
            this.socket.Connect(url);
        };
        /**
         * 关闭
         */
        WebSocket.prototype.Close = function () {
            //启动连接
            this.socket.Close();
        };
        /**
         * 发送消息
         * @param command 命令枚举
         * @param data 数据
         */
        WebSocket.prototype.Send = function (data) {
            if (data === void 0) { data = null; }
            //组装游戏命令Dto
            var gameDto = new Dto.GameMessageDto();
            gameDto.Command = Enum.GameCommand.MSG_GAME_BET;
            gameDto.Data = data;
            //组装消息dto
            var msgDto = new Dto.MessageDto();
            msgDto.Command = Enum.MainCommand.MSG_GAME;
            msgDto.Data = gameDto;
            var msg = JSON.stringify(msgDto);
            //发送
            this.socket.Send(msg);
        };
        /**
        * 连接回调
        */
        WebSocket.prototype.OnConnect = function () {
            //广播上层-连接成功
            this.event(SocketManager.Event.OnConnect);
        };
        /**
         * 关闭链接
         */
        WebSocket.prototype.OnClosed = function () {
            //广播上层-关闭连接
            this.event(SocketManager.Event.OnClose);
        };
        /**
         * 错误
         */
        WebSocket.prototype.OnError = function (message) {
            //广播上层-错误
            this.event(SocketManager.Event.OnError, message);
        };
        /**
         * 正在重连
         */
        WebSocket.prototype.OnWillReconnect = function () {
            //广播上层-重连
            this.event(SocketManager.Event.OnWillReconnect);
        };
        /**
         * 接收消息
         */
        WebSocket.prototype.OnMessage = function (message) {
            // console.log(message);
            if (!message)
                return;
            var messageDto;
            try {
                messageDto = JSON.parse(message);
            }
            catch (error) {
                console.log(error);
                return;
            }
            switch (messageDto.Command) {
                case Enum.MainCommand.MSG_ACK:
                    //广播上层-ACK
                    this.event(SocketManager.Event.OnAck, messageDto.Data);
                    break;
                case Enum.MainCommand.MSG_GAME:
                    //广播上层-游戏命令
                    this.event(SocketManager.Event.OnGame, messageDto.Data);
                    break;
                case Enum.MainCommand.MSG_ERROR:
                    //广播上层-错误
                    this.event(SocketManager.Event.OnError, messageDto.Data);
                    break;
                case Enum.MainCommand.MSG_KICKOUT:
                    //登出，断开连接
                    this.socket.Close();
                    //广播上层-登出
                    this.event(SocketManager.Event.OnLogout);
                    break;
                case Enum.MainCommand.MSG_SYSTEM_PUSH:
                    //系统推送消息
                    this.event(SocketManager.Event.OnSystemPush, messageDto.Data);
                    break;
                default:
                    break;
            }
        };
        return WebSocket;
    }(Laya.EventDispatcher));
    SocketManager.WebSocket = WebSocket;
})(SocketManager || (SocketManager = {}));
//# sourceMappingURL=SocketManager.js.map